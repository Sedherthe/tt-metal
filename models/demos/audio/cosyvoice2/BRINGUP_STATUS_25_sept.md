...
Last updated 2026-09-25, for the next Claude Code session. **Read "STATE AS OF 2026-09-25" first; it supersedes
everything below it.** The older sections (2026-09-18 → 2026-09-24) follow unchanged for history, and several of
their claims are now known to be false (listed below). Treat every claim here as something to re-verify against
`git log`/`git show` and a real test run: this file is a map, not the territory.

**Standing rules (user):** no `git commit` / `git push` unless the user asks for that specific action in that turn;
no `Co-Authored-By` trailer; once a logical chunk is committed and verified, say so immediately so the user can
push (work has been lost to dead instances more than once).

**Bounty:** tenstorrent/tt-metal #54104. Fork `github.com/Sedherthe/tt-metal`. Main branch
`bringup/cosyvoice2-istft` = `5c65be445c` on the fork (verified with `git ls-remote` 2026-09-25). This round's
unverified work is on the WIP branch `wip/cfm-streaming-2026-09-25` (one WIP commit on top of `5c65be445c`), which
is **not** the planned final history (see "Planned 3-commit split").

## STATE AS OF 2026-09-25

### Ground truth re-established at the start of the day

- HEAD was `5c65be445c` (encoder chunk-causal masking/bucketing), already pushed; the older text's "not pushed" is
  stale.
- **Gone, never recovered** (lost with an instance before they were pushed): the CFM streaming round
  (`41f7ed27ff` does not exist), `TtHiFTStreamingState`, and the scripts behind the 09-23/24 numbers.
  `/home/user/cosyvoice2_stft_fix_wavs/` (wavs, unposted progress-note draft) is gone too.
- Full suite at `5c65be445c`: **152/152 passed** on the N150 (34m49s cold on a fresh instance: every kernel
  compiled from scratch).
- A fresh instance needed `uv pip install --python /opt/venv/bin/python "graphviz>=0.20.3"` before `ttnn` would
  import. `onnxruntime`, `torchaudio` and `whisper` are missing (only the eval/WER scripts need them). **Ask the user
  before installing any of them**; `whisper` pulls torch, so it must go through
  `--extra-index-url https://download.pytorch.org/whl/cpu`, then check with `uv pip show torch torchaudio` that both
  still carry `+cpu`.

### Older claims below that are FALSE (do not trust them)

- "CFM round committed `41f7ed27ff`", "153/153": that commit never reached the fork. The CFM round was rebuilt
  today; see below.
- "`TtHiFTStreamingState` BUILT": gone. It never passed verification anyway (three open HiFT findings, below).
- "Single-slot recapture overhead is 99.8% of total CFM time under the real schedule": wrong. With measured costs
  it is **23%**; a full-range cache would save ~19% (see the thrashing simulation).
- "Multi-slot LRU trace cache BUILT and verified": a lazy multi-slot cache is **not allocation-safe**; capacity is
  now fixed at 1 and anything else raises (below).
- "The CFM partial-mask guard exempts streaming because streaming forces the explicit chain": stale reasoning
  (predates masked SDPA). Current rule below.
- "the 0xaa34 message is an unrecognized clock-state message": it is `GET_AICLK`; see "PCIe drops".

### What the WIP branch contains

`tt/flow/decoder.py`, `tests/pcc/test_flow_decoder.py`, `scripts/perf_2026_09_23/bucket_sizing_simulation.py`,
`scripts/perf_2026_09_25/` (4 scripts + README), `wip/commit1_single_slot_trace_safety.patch` (commit 1 of the
planned split, prepared), and this file.

**1. Streaming (chunk-causal) mode in the flow estimator + traced CFM solver** (`tt/flow/decoder.py`)

- Masking exactly as upstream builds it (`CausalConditionalDecoder.forward` → `add_optional_chunk_mask` →
  `mask_to_bias`, re-read from the fetched source): `padding_mask & subsequent_chunk_mask(T, 50)`, then
  `(1 - m) * -1e10`. `static_chunk_size = 50 = encoder.CHUNK_SIZE_UP`; the helpers
  (`subsequent_chunk_mask_torch`/`chunk_causal_bias_torch`) are imported from `tt/flow/encoder.py`, not re-derived.
  The torch reference builds the boolean AND literally; the device builds the additive form
  `pad_bias[B,1,1,T] + chunk_bias[1,1,T,T]`.
- Fused SDPA takes that full `[B,1,T,T]` bf16 DRAM bias as `attn_mask`. The op rejects a key-only `[B,1,1,T]`
  padding row, so the padding term has to be summed in. Non-streaming still passes `attn_mask=None`.
- **What is refreshed on trace reuse:** the chunk term is `slot.chunk_bias_buf`, uploaded once at capture (a pure
  function of `t_len`, and `t_len` is in the trace key). The padding term is rebuilt inside the traced body every
  replay from `slot.mask2_buf`, which `_reuse_trace` refills from each solve's mask. The sum is computed inside the
  body every replay, never stored. So a reused trace at the same bucket and a different valid length masks the new
  padding correctly.
- Trace key is `(t_len, channels, streaming)`: a streaming trace never serves a non-streaming solve, or vice versa.
- **Partial-mask guard (worked out from the current code path):** raise only for non-streaming with fused SDPA on,
  the one path whose attention never sees the mask. Streaming applies the full bias on both attention paths, and
  the non-streaming explicit chain adds the padding row, so neither is refused.
- Why the non-aligned cases matter: upstream's `prompt_token_pad` makes every mid-stream valid length a multiple
  of 25 tokens (chunk-aligned). Only each utterance's final `finalize=True` call (all remaining tokens) has an
  arbitrary length, and that is where the padding term matters. At aligned lengths the chunk term alone already
  hides every padded key, so aligned test points cannot detect a missing padding term.

**2. Trace cache: capacity fixed at 1 (user decision 2026-09-25)**

- `COSYVOICE2_CFM_TRACE_CACHE_CAPACITY` / `trace_cache_capacity`: only 1 is accepted; anything else raises
  `ValueError` at construction. `_traces` is still keyed and LRU-ordered so a safe design could slot in later.
- **The multi-slot safety finding.** Run under `TT_METAL_TRACE_ALLOC_TRACKING=1` (set before importing ttnn), a
  lazy LRU with capacity 2 failed at A's replay after B was captured. The tracker listed all of B's persistent slot
  buffers (conditioning, `x_buf`, `temb`, `dt`, B's trace output) as allocated while A was live, so A's replay may
  overwrite them. At toy sizes the PCCs still pass, because B's slot buffers are refilled before B's own replays.
  In a real session, B's warm-up would also create new-bucket conv weights and program-cache entries while A is
  live, and those are never refilled. The lost 09-23 test (A→B→A) never replayed B afterwards, so it could not see
  this.
- A safe multi-trace design would warm every geometry and capture every slot up front, before any persistent
  allocation that must survive a replay. **Not built; the user chose not to build it now.** What it would buy is
  at most ~19% of CFM time over back-to-back utterances.

**3. Two pre-existing single-slot trace bugs + the fallback fix** (found 2026-09-25; present on HEAD)

The allocation tracker failed every traced CFM test on HEAD's code. PCC checks had not caught either bug.

- **dt copy kernel compiled after capture.** `dt_buf` was created with `from_torch`, so the first `dt`-shaped
  `ttnn.copy` program was compiled during the first replay loop, allocating a program-cache device buffer while the
  trace was live. Fix: create `dt_buf` as zeros and `ttnn.copy` the first `dt` in before capture; also warm the
  four conditioning refills `_reuse_trace` runs (clone+copy each buffer onto itself) before capture.
- **Initial-noise buffer lifetime.** `z_dev` (the initial noise upload) was allocated while the cached trace was
  live (on reuse) and freed only after the replay loop. Fix: free it before the first replay (it is only a copy
  source by then).
- **Failed-capture fallback freed tensors it reads.** When capture failed, `_release_trace()` deallocated the
  conditioning tensors the failed capture had adopted, and the eager fallback then read those freed tensors. Fix:
  hand them back before releasing the rest.
- With the first two fixed, every capacity-1 traced test passed the tracker on the N150, including the
  pre-existing cross-utterance reuse test and the new streaming reuse test.

**4. Tests** (`tests/pcc/test_flow_decoder.py`; 12 at HEAD)

New: `test_streaming_reference_is_chunk_causal` (host), `test_cfm_partial_mask_guard` (host),
`test_device_streaming_decoder_bucketed_matches_exact_length[128-100 / 128-70]`,
`test_device_cfm_streaming_traced_reuse_refreshes_padding`,
`test_cfm_trace_cache_capacity_other_than_one_is_refused` (host),
`test_device_cfm_trace_capture_failure_falls_back_to_eager`,
`test_device_cfm_traces_pass_allocation_tracker`.

**5. Scripts** (`scripts/perf_2026_09_25/`, README inside; recreations of the lost 09-23/24 scripts, re-run)

`masked_sdpa_investigation.py`, `cfm_streaming_real_checkpoint_check.py`,
`cfm_trace_cache_thrashing_simulation.py`, `nonstreaming_timing_old_vs_new.py`; plus `real_hop_schedule_lengths` /
`session_lengths` and PART 3 in `scripts/perf_2026_09_23/bucket_sizing_simulation.py`.

### Verified on hardware (N150, 2026-09-25) vs. written but NOT run

**Verified on hardware:**

- Full suite at `5c65be445c` (before any of this round's changes): 152/152.
- `masked_sdpa_investigation.py` (op level, `[2,8,T,64]` bf16, q128/k256):
  - The device broadcast sum equals the host mask.
  - PCC 0.9997 vs float64 at T 384/768/1536, aligned and non-aligned.
  - Masked SDPA is 2.2× / 4.9× / 7.1× faster than the explicit chain.
  - A mask costs 2.1–3.5× over no mask.
  - The op's default program config is slower than q128/k256 at T 768/1536 (slightly faster at 384).
  - Leak probe (large values in padded V rows): a chunk-only mask collapses to PCC 0.13 / 0.30 / 0.76 at
    310→384 / 660→768 / 1510→1536; the full mask stays at 0.9997. With random V, the missing padding term still
    scores > 0.99.
- `test_flow_decoder.py`: the 12 existing tests plus the then-6 new tests passed on device, all PCC ≥ 0.999:
  - bucketed decoder vs exact length 0.9990 / 0.9993;
  - chunk-only negative control 0.939, max|diff| 2.5 vs 0.15;
  - traced streaming reuse 100→110 in the 128 bucket 0.9996;
  - eager streaming 0.9996.
- The tracker pass (`TT_METAL_TRACE_ALLOC_TRACKING=1`) on all traced tests: clean at capacity 1 after the two
  fixes; capacity 2 failed as described.
- `cfm_streaming_real_checkpoint_check.py`, **first version** (real `flow.pt`):
  - At all six (valid, bucket) pairs (300/310→384, 650/660→768, 1500/1510→1536), the TT bucketed solve was
    bit-identical to the TT exact-length solve (PCC 1.0, max|diff| 0).
  - Against the fp32 torch chunk-causal reference: 0.9962–0.9988.
  - In that version the non-aligned cases did NOT reuse the aligned case's trace (an exact-length solve in between
    evicted it), so real-weight padding refresh on reuse was not exercised there. The unit test does cover it.
  - Timing, traced 10-step solve (ms):

    | bucket | capture | recapture (warm) | streaming steady | ms/step | non-streaming steady |
    |---|---|---|---|---|---|
    | 384 | 734.8 | 744.4 | 501.6 | 50.16 | 406.6 |
    | 768 | 1069.6 | 1071.9 | 820.4 | 82.04 | 733.4 |
    | 1536 | 2071.7 | 2063.9 | 1628.4 | 162.84 | 1377.7 |

    1536 exceeds the 1 s per-chunk budget; that is measured, not extrapolated. It matches the lost doc's
    508.4 / 831.2 / 1642.2.
- CPU-only, run: `cfm_trace_cache_thrashing_simulation.py` (measured costs), `bucket_sizing_simulation.py` PART 3
  (per-utterance hop reset reproduces the doc's 9 calls / 80.0% hit rate / 96.8 MB breaking point).
- `pre-commit` passes on every touched file.

**Written but NOT run on hardware** (the card died before these):

- The **fixed** `cfm_streaming_real_checkpoint_check.py`: bucket solves back to back, so the non-aligned case and
  the chunk-only control reuse the aligned case's trace; the control compared against TT exact; a non-streaming
  TT-vs-torch baseline per aligned length.
- `nonstreaming_timing_old_vs_new.py`: the user's point 3, that non-streaming TIMING is unchanged, not just the
  numbers. It loads `decoder.py` from `5c65be445c` via git and times old vs new, eager and traced, at 384/768/1536.
- `test_device_cfm_trace_capture_failure_falls_back_to_eager`: never run. Also planned: run it against unfixed HEAD
  to show it fails there.
- **`test_device_cfm_traces_pass_allocation_tracker` (the pending standalone tracker test)**: never run. It re-runs
  this file's traced CFM tests in a subprocess with `TT_METAL_TRACE_ALLOC_TRACKING=1`. A subprocess because both the
  C++ runtime options and the Python `execute_trace` wrapper read the env var once at process start, and the repo
  root `conftest.py` imports `ttnn` first, so it cannot be switched on inside a running pytest process. **Open
  questions:** its cost, and whether the child can open the card while the parent pytest process's MetalContext
  still holds the UMD driver. If either is a problem, say so rather than keep it.
- The capacity-must-be-1 change was made after the last device run. Its host test passes; the device tests were
  not re-run after it (the change only touches `__init__` validation and docstrings).
- **The full suite after this round's changes was never run.** Expected count: 152 + 8 = 160 (12 → 20
  collected items in `test_flow_decoder.py`: 7 new test functions, one parametrized ×2). Verify.

### Planned 3-commit split (onto `bringup/cosyvoice2-istft`, replacing the WIP commit)

Commit and verify each in order; tell the user after each so they can push.

1. **Trace-safety fixes to the existing single-slot CFM trace**: `wip/commit1_single_slot_trace_safety.patch`
   (`git apply --check --cached` clean against `5c65be445c`). HEAD's single-slot code plus only the three fixes
   (dt kernel compiled before capture, noise buffer freed before replay, fallback not freeing what it reads), plus
   the two new tests (fallback, tracker subprocess). Verify: those tests on the fixed code AND failing on unfixed
   HEAD, the four existing traced tests under the tracker, then the full suite.
2. **Streaming chunk-causal masking + guard + tests**: the WIP branch's `decoder.py` / `test_flow_decoder.py` on
   top of commit 1 (includes capacity == 1 enforcement and the LRU-keyed `_traces`). Verify: the whole
   `test_flow_decoder.py` on device, the tracker subprocess test, the fixed real-checkpoint check,
   `nonstreaming_timing_old_vs_new.py`, the full suite.
3. **`scripts/perf_2026_09_25/` + `bucket_sizing_simulation.py`**: update the README's "Not yet run" section with
   the real results first. Drop `wip/` from the tree.

The user authorized these three commits on 2026-09-25 contingent on each being verified. Re-confirm with the user
in the new session before committing, since approval does not carry across turns by default.

### Hop-reset decision (user, 2026-09-25)

Upstream `CosyVoice2Model.tts` never resets `self.token_hop_len`: it grows 25→50→100 in the first streaming
utterance and stays at 100 for every later utterance on the same model object, with `prompt_token_pad` then
computed against 100. **Our streaming path must reset the hop to 25 per utterance**, because of the
TTFP < 500 ms target. Both behaviors are simulated (`session_lengths(persist_hop=...)`): upstream-literal gives 12
distinct buckets and a 70.7% hit rate vs 9 and 80.0% for the per-utterance reset.

### PCIe drops (hardware, unresolved)

- **This pod (2026-09-25):** after `cfm_streaming_real_checkpoint_check.py` finished, it had released all traces
  and `ttnn.close_device` had returned normally. Then process-exit teardown
  (`MetalContext::destroy_all_instances` → UMD `Cluster::close_device` → `set_clock_state`) hit `PcieHangError`:
  "Read 0xffffffff over PCIe ID 5: the board should be reset". `TT_METAL_TRACE_ALLOC_TRACKING` was off. The run
  used `trace_region_size=200_000_000`.
  - `tt-smi -r` and `tt-smi -r /dev/tenstorrent/5 --use_luwen` both failed ("Secondary bus reset not completed",
    "Post-reset actions did not complete"). The link stayed down; the user confirmed it is dead on this pod.
- **The previous pod was lost the same way (per the user).**
- **Versions:**
  - KMD **2.9.0**: meets UMD's minimum 2.0.0 and INSTALLING.md's ≥ 2.8.0.
  - Firmware bundle **19.11.0**: inside UMD's compatible range (Wormhole min 18.3.0) but newer than UMD's latest
    fully tested version, 19.7.1 (`get_latest_supported_firmware_version`; UMD notes that only at debug level).
  - tt-smi 6.1.0; `TT_METAL_VERSION=v0.77.0`.
  - KMD module parameters: `power_policy=Y`, `idle_power_down_grace_ms=5000`, `auto_reset_timeout=10`,
    `reset_limit=10`.
- The log line "message code 0xaa34 not recognized by FW" is **not** a version mismatch: `0xaa34 = 0xaa00 |
  GET_AICLK (0x34)`, and UMD prints "not recognized" whenever the status read returns `0xffffffff`. The card was
  already off the bus.
- **Root cause not established.** Unverified leads: firmware newer than UMD has tested; KMD `power_policy=Y` with a
  5 s idle power-down grace (device idle during the long host-side torch reference sections, then touched again).
  Worth asking infra before the next long run.
- A zombie `[python] <defunct>` (the killed-timeout pytest from the Step 1 suite run) lingered; it is harmless.

### HiFT streaming: still open (not started this round)

Unchanged from the 2026-09-24 section below, and still unresolved: (1) the bf16-generator + fp32-decoder dtype
crash in `TtStft`'s concat; (2) the 0.977 PCC gap in plain non-streaming `TtHiFTGenerator.inference()` with real
weights; (3) streaming PCC 0.988 / 0.959 / 0.940 degrading, not yet separated from (2). The user's plan for (2):
first confirm the source module's noise draws really are shared between torch and TT, then measure the gap at
`mel_frames` 16, 108 and 208 (the real streaming geometries) to see whether it is edge-dominated at 16.

### Environment (this instance)

Run device jobs from outside the repo root:
`PYTHONPATH=/home/user/tt-metal/ttnn:/home/user/tt-metal/tools:/home/user/tt-metal timeout -s KILL N /opt/venv/bin/python ...`.
`HF_HOME=/home/user/models` (checkpoints download on first use). Do not wrap a long pytest in a `timeout -s KILL`
shorter than it needs (cold kernel compile made the suite take 35 min); a SIGKILL mid-device-op can wedge the card.

---

# HISTORY — BRINGUP_STATUS_24_sept.md, unchanged (superseded by the 2026-09-25 section above)
...
Committed as `5c65be445c`, not pushed.

### Streaming design, CFM round: real chunk-causal masking + bucketing for the traced Euler solver

Same discipline as the encoder round, applied to `tt/flow/decoder.py`'s traced CFM
solver: real upstream masking logic ported verbatim (not reinvented, reused directly from
`tt/flow/encoder.py`), real-checkpoint verification at real bucket boundaries, a design
question modeled with real numbers before writing code. Four items, in the order asked:

**Item 1 -- does the CFM's single-slot trace cache thrash under bucketing? Modeled, real
numbers, answered before writing item 2's code**
(`scripts/perf_2026_09_23/cfm_trace_cache_thrashing_simulation.py`, pure Python, reuses
the encoder round's real bucket-access trace directly -- the CFM's own geometry is the
encoder's up-rate bucket, `token-rate bucket x 2`, not a separate derivation). Real
measured costs (`trace_lifecycle_experiment.py`, 2026-09-22): recapture at an
already-kernel-compiled shape 697.7 ms; steady-state replay (cache hit) 498.8 ms/10-step
solve; release ~1.6 ms.

- **Single-slot (today's design) vs. an LRU sized to the full distinct-bucket range**,
  5x back-to-back 30s utterances (13 distinct mel-rate buckets/utterance): single-slot 65
  recaptures (45.35 s pure recapture overhead); LRU (capacity=13) 13 recaptures (9.07 s).
  **Single-slot pays 5x as many recaptures.**
- **Sharp threshold, not a smooth curve**: an LRU capacity sweep (1/2/4/8/13) shows
  capacities 1-8 give ZERO benefit over single-slot -- identical recapture count. Only
  capacity=13 (the full range) helps. Because one utterance's own walk visits each of its
  13 buckets exactly once, monotonically increasing, never revisiting within that walk, a
  partial-capacity cache has always evicted the early (small) buckets by the time the
  SAME utterance's walk reaches its own end, so the NEXT utterance's first (small) bucket
  is still a miss regardless -- only full-range capacity lets utterance N+1 hit what
  utterance N already captured.
- **Answer: yes, and it's now BUILT** -- `TtCausalConditionalCFM._traces` (an
  `OrderedDict` of `_CfmTraceSlot`, LRU-evicted, `COSYVOICE2_CFM_TRACE_CACHE_CAPACITY`,
  default 1 -- exactly the original single-slot behavior). Verified 2026-09-23 (same
  turn as the fix below, built regardless of the item 3 re-measurement per explicit
  instruction: "that finding stands on its own"): a new `test_device_cfm_multi_slot_
  trace_cache` proves capacity=1 forces recapture on A/B/A (matches the ORIGINAL,
  pre-existing `test_device_cfm_trace_cache_across_utterances_and_replays`'s case D
  exactly) and capacity=2 lets A survive B's capture (a real cache hit, not just fewer
  recaptures in principle). A real design also needs sizing against `trace_region_size`
  (a fixed, separate memory pool from ordinary DRAM -- N resident traces means N times
  the persistent-buffer footprint) -- NOT bounded here; setting the capacity too high
  surfaces as a real allocation failure at capture time, not a graceful fallback.

**Item 2 -- real chunk-causal masking ported into the estimator's attention**
(`tt/flow/decoder.py`), reusing `chunk_causal_bias_torch`/`CHUNK_SIZE_UP` from
`tt/flow/encoder.py` directly, not reinvented. Confirmed from the real checkpoint's own
`cosyvoice2.yaml` (not assumed): `estimator.static_chunk_size = chunk_size *
token_mel_ratio = 50` (== `CHUNK_SIZE_UP`, matches the encoder's up-rate stage exactly)
and `estimator.num_decoding_left_chunks = -1` (the class's own `2` default is overridden
by the real yaml -- moot either way, since the real, non-deprecated `subsequent_chunk_
mask` ignores `num_left_chunks` entirely regardless of what's passed). Unlike the
encoder, no `valid_length`-style decoupling is needed: every causal conv in this
estimator is left-padded only (reads backward, never into padding), so the existing
`mask` argument (built by the caller) already correctly handles bucket padding for the
non-attention legs; only the attention bias needed the new chunk-causal term, ADDED to
(not replacing) the existing padding-derived one.

- `CausalConditionalDecoderRef.forward`/`TtCausalConditionalDecoder.__call__`/
  `_forward_from_raw_temb` gain a `streaming`/`chunk_bias` param. `streaming=False`
  (default) is unchanged.
- **Corrected same-day, after the user pushed back on an unexamined assumption**: the
  pre-existing docstring's claim that fused SDPA "assumes an all-ones mask" turned out to
  be this IMPLEMENTATION never wiring `attn_mask` through, not a real limitation of the
  op -- `ttnn.transformer.scaled_dot_product_attention`'s own docstring documents
  `attn_mask (ttnn.Tensor, optional)... Shape [b x nqh x s x s]`, additive-bias
  semantics. Investigated directly (`scripts/perf_2026_09_23/
  masked_sdpa_investigation.py`, op-level, real `SDPAProgramConfig`
  q_chunk=128/k_chunk=256 -- an earlier draft of this investigation omitted the program
  config and got a wildly misleading "SDPA is 27-40x slower" result before that was
  caught and fixed): masked SDPA is CORRECT (PCC 0.999957-0.999961 vs. a float64
  reference) and **2.2x-4.4x FASTER than the explicit chain** at T=384/768 -- the
  opposite of the original design choice. `TtBasicTransformerBlock.__call__`/`_sdpa` now
  pass the real chunk-causal bias through to fused SDPA as `attn_mask` (a new `streaming`
  param controls this; `use_sdpa` remains as an explicit override/escape hatch to the
  slower explicit chain if ever needed). The non-streaming default still passes
  `attn_mask=None` (measured faster than passing an all-zero mask -- SDPA's own kernel
  skips the mask-add when the argument is absent), so it stays exactly as fast as before.
- Folded into the traced solver: `TtCausalConditionalCFM._capture` builds a new
  `_chunk_bias_buf` (host-computed `chunk_causal_bias_torch(t_len, CHUNK_SIZE_UP)`,
  uploaded ONCE -- a pure function of `t_len`, constant across every replay and reuse at
  that geometry, so `_reuse_trace` never needs to refresh it, unlike the per-step temb/dt
  buffers). `(t_len, ch, streaming)` is now the trace key -- a streaming trace's captured
  body genuinely differs from a non-streaming one at the same shape and must never be
  reused for the other.
- `TtCausalConditionalCFM.forward`'s existing partial-mask guard (raises when fused SDPA
  is on and the mask isn't all-ones) now exempts `streaming=True` calls, since those
  correctly force the explicit chain regardless of the global `flow_fused_sdpa()` setting.

**Item 3 -- real per-chunk cost of the traced, masked, bucketed solver, real bucket
sizes, RE-MEASURED after the masked-SDPA fix above** (same script, same real bucket
pairs, real checkpoint weights, real traced solve):

| bucket (mel) | steady-state 10-step solve (explicit chain, superseded) | steady-state 10-step solve (masked SDPA, current) | improvement | ms/Euler-step (current) |
|---|---|---|---|---|
| 384 | 662.4 ms | **508.4 ms** | 23% faster | 50.84 ms |
| 768 | 1483.6 ms | **831.2 ms** | 44% faster | 83.12 ms |
| 1536 | (not measured pre-fix) | **1642.2 ms** | -- | 164.22 ms |

**The earlier "ceiling" finding is superseded at bucket=768, not just softened**: the
CFM's own steady-state cost there is now 831.2 ms -- back UNDER the 1-second-per-chunk
real-time budget (was 1483.6 ms, well over it, under the unexamined explicit-chain
design). This was a real measurement error compounding a real design mistake (forcing
the slower path), not a fundamental property of masked/bucketed attention at these
sizes -- see the SDPA investigation above for how it was found and fixed.

**bucket=1536 measured directly (2026-09-23), not extrapolated -- a real ceiling DOES
exist, confirmed, just further out than the pre-fix number suggested.** An earlier draft
of this section extrapolated 384/768's growth ratio (1.63x per doubling) to predict
125-135 ms/step at bucket~1536 -- flagged explicitly at the time as an unverified guess,
not to be treated as confirmed. The real measurement (token-rate valid_length=750,
bucket_length=768, mel valid=1500, mel bucket=1536; real checkpoint, real traced+masked
solve, PCC 1.0 vs. the exact-length chunk-causal reference) came in at **164.22 ms/step,
1642.2 ms/10-step solve** -- higher than the extrapolation (growth from 768->1536 was
1.98x, close to linear, not the sub-linear 1.63x the first doubling showed; the
sub-linear behavior at 384->768 did not continue). **This genuinely exceeds the
1-second-per-chunk budget** -- the concern is real and now measured, not guessed, at
this bucket size. See "does the bounty define an expected streaming utterance/session
duration" immediately below for whether this bucket size is ever actually reached in a
real streaming session -- that question, not this measurement, is what's still open.

**Item 4 -- real-checkpoint PCC verification at real bucket boundaries, against a genuine
chunk-causal reference** (same script as item 3; ground truth = the SAME masked, traced
solve run at the EXACT valid length, no bucket padding -- the encoder round's mistake
explicitly avoided: never compare against full/non-causal attention):

| valid_length | bucket_length | PCC (explicit chain, superseded) | PCC (masked SDPA, current) | max\|diff\| (current) |
|---|---|---|---|---|
| 300 | 384 | 0.999986 | **1.0** | 0.0 |
| 650 | 768 | 0.999813 | **1.0** | 0.0 |

Now bit-exact, not just comfortably above gate -- re-measured after the masked-SDPA fix
above (same script). Both clear the 0.99 gate with a wide margin either way. Reference
self-consistency (2 reps, same trace) confirmed PCC 1.0 before trusting the comparison
against it. **Bucketing itself is a clean no-op at the valid positions**, as reasoned in
item 2's design (causal convs never read into padding; the chunk-causal mask already
excludes padding from attention) -- verified, not just assumed.

### Real growing hop schedule -- re-checked the encoder/CFM cache-hit numbers, they do NOT hold as stated

HiFT streaming research (2026-09-24, `cosyvoice/cli/model.py`'s `CosyVoice2Model`) found
the real streaming chunk hop is NOT the flat 25 tokens/chunk both `bucket_sizing_
simulation.py` (encoder round) and `cfm_trace_cache_thrashing_simulation.py` (CFM round)
assumed -- it GROWS: `token_hop_len` starts at 25, doubles after every chunk
(`stream_scale_factor=2`), capped at `token_max_hop_len=4*25=100`. Real sequence:
**25(+prompt_pad) -> 50 -> 100 -> 100 -> 100 -> ...**. Re-ran both simulations with the
corrected schedule (`real_hop_schedule_lengths`, now in `bucket_sizing_simulation.py`,
imported directly by the CFM simulation rather than re-derived) instead of assuming the
old numbers still applied -- confirmed, not assumed, per explicit instruction.

**Encoder round (`GeometryWeightCache`), linear step=64, 5x 30s utterances:**

| | flat-25 (superseded) | real 25->50->100 schedule (current) |
|---|---|---|
| chunks/30s utterance | 30 | **9** |
| distinct buckets/30s utterance | 13 | **9** |
| cross-utterance hit rate | 91.3% | **80.0%** |
| breaking point (per-bucket MB before eviction hurts) | 67.0 MB | **96.8 MB** |
| margin vs. pessimistic estimate (14.4 MB/bucket) | 4.65x | **6.72x** |

**Hit rate is genuinely LOWER (91.3% -> 80.0%), not just relabeled** -- fewer total calls
per utterance (9 vs. 30) means fewer chances to hit a bucket a previous utterance already
warmed. Partial-capacity-gives-zero-benefit still holds (capacities 1/2/4/8 all still
match capacity=1 exactly). Breaking-point margin actually IMPROVED (fewer distinct
buckets per utterance = more DRAM budget per bucket before eviction thrashing starts) --
the DRAM-safety conclusion still holds and is now if anything more comfortable; the
cache-hit-rate conclusion does not hold as originally stated and needed this correction.

**CFM round (`TtCausalConditionalCFM`'s multi-slot LRU), same real schedule, 5x 30s
utterances:**

| | flat-25 (superseded) | real 25->50->100 schedule (current) |
|---|---|---|
| chunks/30s utterance | 30 | **9** |
| distinct mel-rate buckets/30s utterance | 13 | **9** |
| single-slot WITHIN-utterance same-bucket hits | some (buckets revisited ~2-3 consecutive chunks each) | **ZERO** -- every chunk in a 30s utterance is now a distinct bucket |
| single-slot vs. LRU recapture ratio | 5.00x | 5.00x (unchanged numerically, but the underlying story is worse -- see below) |
| pure recapture overhead saved by the LRU, 5 utterances | 10.45 s | 7.23 s (fewer total chunks, so smaller absolute number, but... |
| ...as a FRACTION of total single-slot time | 45.35s/87.85s = 51.6% | **31.40s/31.47s = 99.8%** |

**This is a bigger correction than a relabeling.** Under the real schedule, because the
hop grows so fast, single-slot gets ZERO within-utterance reuse at all -- previously it
benefited from a bucket staying resident across ~2-3 consecutive same-bucket chunks
before the walk advanced; now essentially every chunk lands in a new bucket, so
single-slot's own recapture overhead is now ~100% of its total time, not ~52%. **The case
for the multi-slot LRU cache is stronger under the real schedule, not weaker** -- good
news given it's already built, but the earlier "5x" framing understated how much
single-slot alone would have cost in practice. Partial-capacity-gives-zero-benefit still
holds identically (a sharp threshold, confirmed again under the corrected schedule).

**Scripts updated in place** (`bucket_sizing_simulation.py`,
`cfm_trace_cache_thrashing_simulation.py`) rather than left showing the superseded
flat-25 numbers -- both now import/use `real_hop_schedule_lengths`, the single source of
truth for the real schedule, so a future re-run of either always reflects the same
correction.

### Running list: host round-trips still in the streaming path

Started 2026-09-24, per explicit instruction, to track everything still off-device
before any of this can be called a real streaming loop rather than a chain of
host-mediated stages. Add to this list, don't let entries go stale silently:

1. **`TtRelPositionMultiHeadedAttention._rel_shift`** (`tt/flow/encoder.py`) -- once per
   Conformer layer (10 layers/call), `ttnn.to_torch` -> real upstream `rel_shift` in
   torch -> `ttnn.from_torch` back. This is ALSO why the flow encoder cannot be traced at
   all (see that class's docstring) -- not just a streaming-loop concern, a correctness-
   path cost paid on every single call, streaming or not.
2. **HiFT's audio-domain Hamming crossfade** (`fade_in_out`, real upstream
   `cosyvoice/utils/common.py`) -- planned, not yet built, to run host-side (numpy/torch)
   given the tiny tensor size (`source_cache_len=3840` samples) rather than a device
   kernel. Decided 2026-09-24, see the HiFT streaming design section below.
3. **`TtCausalConditionalDecoder._sinusoidal_pos_emb`** (`tt/flow/decoder.py`) -- computed
   on the HOST every call (cheap: a length-`2B` vector, not a mel-length tensor), NOT a
   round-trip in the sense of reading device data back (no `ttnn.to_torch` of an
   activation), just host-side math before upload. Listed for completeness since it's
   still host compute inside the per-step path; genuinely cheap, unlike #1.
4. **(Anticipated, not yet real -- HiFT isn't traced today)** if/when `TtHiFTGenerator`'s
   decode path is ever traced, `TtHiFTStreamingState`'s carried tensors (`mel_cache`,
   `source_cache`, `speech_cache`) are at real risk of the SAME trace-buffer-corruption
   class this bring-up's own CFM round already found and fixed once this session (PCC
   ~0.6 from device buffers allocated/held alive while a trace was active -- see
   `TtCausalConditionalCFM`'s docstring and `cosyvoice2_trace_reuse_hazard.md`). The
   CosyVoice1 reference port (`/home/user/reference-cosyvoice1`) hit this EXACT bug for
   its own analogous `StreamState` (`mel_overlap`/`hift_mel`/`hift_source`/`hift_speech`)
   -- see "CosyVoice1 reference cross-check" below -- and their only known remedy (host
   round-trip between chunks) reportedly hangs a DIFFERENT test on a different board.
   Flagged here in writing, matching the "document, don't solve" instruction, since it
   isn't a live problem until HiFT tracing is actually attempted.

### CosyVoice1 reference cross-check (2026-09-24, `/home/user/reference-cosyvoice1`)

Asked explicitly before implementing `TtHiFTStreamingState`: does the CosyVoice1 TT
port's own streaming implementation (`models/demos/cosyvoice/tt/streaming.py`,
`docs/VALIDATION.md`, `PERF.md`) inform the mel/source splice design, and what do their
own streamed-PCC and Wormhole-hang findings say. Real, different codebase/config
(`TOKEN_RATE=50`, `SAMPLE_RATE=22050`, `HOP=256`, vs. CosyVoice2's 25 Hz/24000 Hz/480 --
numbers do NOT transfer, only the mechanism is being compared).

**Their mel/source splice -- confirms the planned approach, on-device throughout, no
host round-trip.** `TtStreamingSynthesizer._fade` does the crossfade with four ops:
`ttnn.slice` splits the new signal into head/rest, `ttnn.multiply`+`ttnn.add` blends the
head against the saved tail with the two half-windows, `ttnn.concat` reassembles. No
in-place assignment anywhere -- confirms the planned `ttnn.slice`+`ttnn.concat`
reconstruction (open question #1 in the design above) is the right general technique,
not a workaround to second-guess. One real difference worth noting, not a discrepancy to
resolve: CosyVoice1 crossfades MEL at the chunk boundary too (`mel_overlap`, 34 frames,
"chunk boundaries are where the flow decoder's context changes, so the mel jumps
slightly"), which CosyVoice2's real upstream (fetched and confirmed in the CFM round)
does NOT do -- only `mel_cache` (prepend, no fade) and the audio-domain fade. Plausible
reason, not confirmed: CosyVoice2's flow decoder's chunk-causal masking (unlimited left
context, identical masking scheme every chunk, already verified this round at PCC
0.999986-1.0) may not produce the same visible context-boundary "jump" CosyVoice1's own
flow decoder does, making the extra mel crossfade unnecessary there. Not chased further
-- CosyVoice2's real source is the authority for CosyVoice2's own design, already
confirmed directly.

**Streamed PCC ~0.90 -- a DIFFERENT kind of bar than this bring-up's usual 0.99, and
correctly so, not a lowered target.** `test_device_streamed_matches_non_streamed`
compares CONCATENATED streamed audio against full non-streamed (batch) audio for the
SAME tokens -- mel-space PCC `0.901830`/`0.901541`/`0.902374` across their three boards,
reported as the PASSING, accepted result. This is NOT comparable to this bring-up's
0.99 gate (`GATE_BF16`) or to this round's own 0.999986-1.0 CFM/encoder chunk-boundary
numbers -- those compare a bucketed/masked chunk against an EXACT chunk-causal
reference of the SAME computation (a padding/mechanism-correctness check); CosyVoice1's
~0.90 compares two GENUINELY DIFFERENT computations (chunked, limited-context streaming
vs. full-context offline) that are not expected to match closely by construction. **This
sets the right expectation for an eventual full-pipeline "streamed output vs. batch
output" test on our own port**: something in the CosyVoice1 ballpark (high 0.8s to low
0.9s) should be treated as a PASS for that specific kind of test, not held to 0.99 --
holding it to 0.99 would be miscalibrated, not rigorous. The per-mechanism checks
(mel-cache/source-cache splice correctness, this round's own upcoming HiFT verification)
still target 0.99, same as everywhere else -- these are different questions with
different appropriate bars, not one relaxed and one strict version of the same question.

**Wormhole latency test hangs -- a specific test-harness pattern, not (as far as their
own evidence shows) the real shipped streaming path.**
`test_device_streaming_first_audio_latency` wedges their n300 (Wormhole) board; both
Blackhole boards run it clean. Several candidates ruled out directly (trace region size
384 MB -> 64 MB changed nothing; warm-before-capture ordering already fixed a DIFFERENT,
earlier hang; a `StreamState` host-round-trip fix that cured an unrelated corruption).
Root cause NOT established; the untested, most-likely lead: this SPECIFIC test holds one
decode trace live across FOUR repeated passes, while their actual shipped path
(`synthesize_streaming`) captures and releases per call -- and THAT shipped path is
independently confirmed working correctly on their n300
(`test_device_streaming_generates_the_same_tokens_as_batch` passes there). **Directly
relevant to us**: this reference port also hit a REAL trace-buffer-corruption bug from
their `StreamState`'s carried tensors (`mel_overlap`/`hift_mel`/`hift_source`/
`hift_speech`) being allocated while a trace was live, then clobbered by a later
`execute_trace` -- the SAME bug CLASS this bring-up's own CFM round already found and
fixed once this session (PCC ~0.6, pre-built device tensors held alive during an active
trace). Their only known remedy (host round-trip for those four tensors between chunks)
fixes the corruption but reportedly hangs a DIFFERENT test on a different board -- an
unresolved tradeoff there, not a clean answer to copy. Folded into the host-round-trip
list above as an anticipated (not yet real) hazard for `TtHiFTStreamingState`, since
HiFT isn't traced today.

### Does the bounty define an expected streaming utterance/session duration?

Asked explicitly (2026-09-23), originally to weigh item 3's extrapolated ~27s concern --
now item 3's bucket=1536 finding is a real MEASUREMENT (1642.2 ms/solve, over the
1-second budget), not an extrapolation, so this question is about whether that
confirmed ceiling is ever actually reached, not whether it exists. Fetched the real
issue text directly (`gh`/`curl` against
`api.github.com/repos/tenstorrent/tt-metal/issues/54104`, not assumed from memory):

**No, it does not.** The issue specifies latency/throughput targets only -- "Streaming
inference support with time-to-first-packet < 500ms", "RTF < 0.4 for streaming
synthesis" (Stage 3) -- and one architectural note ("Handle streaming/causal chunking
for the iSTFT overlap-add step specifically -- whole-utterance processing is not
compatible with the streaming design", Stage 2). Nowhere does it state an expected
utterance length, session length, or any other duration figure. **This is the SAME open
gap as the still-unresolved RTF/TTFP definition question already sitting in the
(unposted) progress-note draft** (`/home/user/cosyvoice2_stft_fix_wavs/
issue54104_progress_note_DRAFT.md`, and this file's own earlier note that "the issue
does not define warm vs cold or the utterance set") -- not a new, separate question:
whoever finalizes that progress note should ask the maintainers for utterance/session
duration expectations at the same time as the RTF/TTFP definitions, one combined ask,
not two.

**Flagged explicitly, not assumed**: whether the current whole-growing-prefix design
(unlimited left context, no sliding window) is viable for arbitrary-length streaming is
an open question this bring-up cannot resolve from the issue text alone. If real
streaming sessions are expected to run long (tens of seconds to minutes), the confirmed
bucket=1536 ceiling matters; if sessions are expected to be short (a few seconds to
~15-20s, closer to the non-streaming Stage 1 utterance lengths already tested), it may
never be reached in practice. This needs a decision or an answer from outside this repo
(the bounty poster, or the user's own judgment call) before being treated as either "a
real ceiling to engineer around" or "a footnote" -- neither is assumed here.

**Documentation, not solved, per instruction**: the interleaving-safety hazard
`TtCausalConditionalCFM`'s docstring already flagged (this trace's safety relies on
running strictly after the LLM decode trace releases; streaming will interleave LLM
decode, flow encode, and CFM solve across chunks, breaking that non-overlap assumption)
still holds unchanged by this round's masking work -- the new `_chunk_bias_buf` is just
one more persistent buffer subject to the same rule, not a new hazard type. Item 1's
single-slot-thrashing finding is now ALSO documented in that same docstring, as a
performance (not safety) consideration layered on top of the same interleaving question.

**Full regression: 153/153 pass** (152 previous + 1 new -- `test_device_cfm_multi_slot_
trace_cache`), zero breakage to the existing non-streaming path. Nothing committed yet.

### HiFT streaming design: `TtHiFTStreamingState` (design read, 2026-09-24 -- not yet built as of this section; see below for implementation status)

Real upstream mechanism, confirmed directly from `cosyvoice/cli/model.py`'s
`CosyVoice2Model` (fetched, not assumed) and `cosyvoice/hifigan/generator.py`'s real
`HiFTGenerator.inference`/`decode` (the class CosyVoice2's own `cosyvoice2.yaml`
actually instantiates -- NOT the newer `CausalHiFTGenerator` class the same file also
contains, which the file's own `__main__` block tests against a CosyVoice3 yaml, a
different, newer model this bring-up is not targeting).

Three layered, independent caches carried per-utterance across chunk calls
(`self.hift_cache_dict[uuid]`), each solving a different discontinuity:

- **mel cache** (`mel_cache_len=8` frames): prepended to each chunk's mel INPUT so
  HiFT's own conv/resnet receptive fields have correct left-context at the chunk
  boundary (ordinary boundary-effect padding, same category as the encoder's
  pre-lookahead, but left- instead of right-context, and at the vocoder instead of the
  encoder).
- **source cache** (`source_cache_len = mel_cache_len * 480 = 3840` samples): the NSF
  excitation's phase is a running integral over the WHOLE utterance (the same
  cumsum-based mechanism this bring-up's own SineGen2 work already validated) --
  `cache_source` OVERWRITES the first 3840 samples of each freshly-computed source
  signal with the exact continuation of the previous chunk's, which mel-context alone
  cannot provide (mel-cache fixes the CONV layers' boundary; source-cache fixes the
  OSCILLATOR's phase).
- **audio-domain Hamming crossfade** (`fade_in_out`, real upstream
  `cosyvoice/utils/common.py`, `np.hamming(2*3840)`): a third, independent safety net on
  the rendered WAVEFORM itself, blending this chunk's start against the previous
  chunk's saved tail -- catches any small residual discontinuity the conv
  stack/iSTFT still introduces even with matched mel+source context. Decided host-side
  (see the host-round-trip list above), given the tiny tensor size.

Real chunk schedule (confirmed the same session as the hop-schedule correction above):
`token_hop_len` starts at 25, doubles per chunk (`stream_scale_factor=2`), capped at
`token_max_hop_len=100`; each chunk also needs `+pre_lookahead_len(3)` tokens buffered.

**Design: a new, small, per-utterance class, not new state on `TtHiFTGenerator`.** The
cache tensors are per-utterance/per-session state (matching real upstream's
per-UUID `hift_cache_dict`), not per-model -- `TtHiFTGenerator` holds weights and should
stay reusable across many sequential/concurrent utterances. `TtHiFTStreamingState` is
instantiated fresh per utterance and calls `TtHiFTGenerator`'s EXISTING public
sub-components (`f0_predictor`, `source`, `decoder`) directly, unmodified -- no change
to `TtHiFTGenerator.inference`'s own non-streaming behavior at all:

```
TtHiFTStreamingState: mel_cache, source_cache (device), speech_cache (host)
    chunk(tt_hift, mel_chunk, mel_frames, finalize) -> waveform:
        1. mel_combined = concat(mel_cache, mel_chunk) if mel_cache else mel_chunk
        2. f0 = tt_hift.f0_predictor(mel_combined); s = tt_hift.source(f0)
        3. splice source_cache into the first 3840 samples of s (ttnn.slice + ttnn.concat
           reconstruction -- confirmed the right technique by the CosyVoice1 cross-check
           above, not a workaround)
        4. waveform = tt_hift.decoder.decode(mel_combined, s, ...)
        5. if speech_cache present: waveform = fade_in_out(waveform, speech_cache, hamming_window)  # host-side
        6. save mel_cache = mel_combined[-8:], source_cache = s[-3840:], speech_cache = waveform[-3840:]
        7. if not finalize: waveform = waveform[:-3840]  # held back for next chunk's crossfade
```

**Geometry: confirms no bucketing scheme needed, with real numbers.** HiFT's own input
mel width per chunk (`mel_cache_len` + this chunk's new mel frames -- NOT the whole
growing prefix, since HiFT is incremental, unlike the encoder/CFM) works out to roughly:
chunk 1 (no cache yet) ~50+2·prompt_pad frames; chunk 2: 8+100=**108**; chunk 3 onward
(hop capped at 100): 8+200=**208**, repeating for every remaining chunk; the final
`finalize=True` call: whatever's left (<=208). **3-4 distinct geometries total per
utterance**, with the steady-state one (208) covering the majority of chunks in
anything but a short utterance -- `GeometryWeightCache` (already wired into
`TtConv1d`/`TtConvTranspose1d`) handles this natively, confirmed not assumed.

**Bar for the eventual verification**: per-mechanism correctness (does the splice/cache
reconstruction reproduce the same values as an exact reference at a chunk boundary) is
held to 0.99, same as everywhere else in this bring-up -- see the CosyVoice1 cross-check
above for why this is DIFFERENT from, and not to be confused with, an eventual
full-pipeline streamed-vs-batch test (which should expect something in CosyVoice1's
~0.90 ballpark, not 0.99).

### `TtHiFTStreamingState` implementation status (2026-09-24): BUILT, real-checkpoint verification NOT yet passing -- do not trust or commit

`TtHiFTStreamingState` (mel_cache/source_cache/speech_cache, `chunk()`) is now written in
`tt/hifigan/generator.py`, following the design above exactly: `ttnn.slice`+`ttnn.concat`
splice (no in-place assign), real `MEL_CACHE_LEN=8`, `source_cache_len =
8*upsample_scale`, host-side `fade_in_out` re-derived verbatim from the real fetched
`cosyvoice/utils/common.py` (not paraphrased):

```
mel_overlap_len = window.shape[0] // 2
fade_in[..., :len] = fade_in[..., :len] * window[:len] + fade_out[..., -len:] * window[len:]
```

Also re-confirmed directly from the real fetched `cosyvoice/cli/model.py`
(`CosyVoice2Model.token2wav`, not summarized) that `source_cache` is the excitation `s`
**after** its own splice/overwrite (`tts_source` returned by `hift.inference`, which is
already-overwritten `s` -- see `hift_generator.py`'s `inference`), and `speech_cache` is
the waveform **after** crossfade -- both details this class now matches.

**Real-checkpoint verification (new script,
`scripts/perf_2026_09_23/hift_streaming_real_checkpoint_check.py`) does NOT pass the 0.99
gate, and per this bring-up's own standing rule ("real-checkpoint verification before any
of this is trusted, same bar as everywhere else") this is NOT being treated as done.**
Three real, run findings, not yet root-caused to a fix:

1. **A real, previously-unexercised dtype-mismatch bug**, found first: building
   `TtHiFTGenerator` at `dtype=bf16` (its f0_predictor/source) while `TtHiFTDecoder` is
   built at `dtype=fp32` (required for real weights, see the checkpoint section above)
   crashes with a `TT_FATAL` inside `TtStft`'s internal `ttnn.concat` ("All Tensors should
   have same dtypes") -- `TtStft` builds its own zero-pad tensors at the decoder's
   configured dtype, and the excitation `s` it receives comes in at whatever dtype the
   generator's `source` module used. No existing test exercises this combination
   (`test_hift_generator_inference.py` always uses one dtype for both). Worked around in
   the verification script by using fp32 throughout rather than fixed generically (out of
   scope for this round) -- **a real bug, flagged here, not fixed**.

2. **With fp32 throughout, a real PCC gap exists in the plain, non-streaming
   `TtHiFTGenerator.inference()` composition itself -- with ZERO caching/splicing
   involved.** Isolation check (`/tmp/hift_full_composition_isolation.py`, real
   checkpoint, fp32, mel_frames=16, single non-chunked call, no streaming code at all):
   PCC **0.977** against the torch reference -- below the 0.99 gate. This is a genuinely
   new finding: `test_hift_checkpoint.py` verifies `decode()` alone at real weights
   (0.999+), `test_f0_predictor_checkpoint.py` verifies `f0_predictor` alone at real
   weights (0.996-0.999), but **no existing test composes f0_predictor -> source ->
   decode together with real checkpoint weights** (`test_hift_generator_inference.py`'s
   only device test is random-init). This gap predates and is independent of the
   streaming work in this round.
3. **The streaming path itself, on top of that**: `TtHiFTStreamingState.chunk()` across 3
   real chunks (mel_frames 16/20/12, growing-hop-shaped, last one `finalize=True`) measured
   PCC **0.988 / 0.959 / 0.940** against a torch reference of the identical chunked
   algorithm (`torch_streaming_hift`, same mel-cache/source-cache/crossfade mechanism,
   same real weights, same shared `sine_noise` draws) -- degrading chunk over chunk. **Not
   yet isolated** whether this is purely inherited/compounded from finding 2's
   pre-existing composition gap (each chunk re-runs the full f0->source->decode
   composition, so a per-call systematic bias could compound across repeated splicing) or
   a genuine additional bug specific to the cache-splice mechanism -- needs finding 2
   fixed or at least understood first, since it's the confound sitting underneath.

**Not committed. Not treated as working.** Per the standing rule, stopping here to report
these three findings rather than continuing to debug unsupervised -- finding 2 in
particular looks like the right place to start (it's upstream of, and a plausible
explanation for, finding 3), but is a distinct investigation from what this round set out
to build, and wasn't flagged as an open risk in the design above.
...
boundary-risk check), 2 (encoder-level chunking/bucketing, committed `5c65be445c`), and
this CFM round (masking ported and verified, multi-slot LRU trace cache BUILT and
verified, masked-SDPA correction found and fixed, item 3's bucket=768 finding
superseded, bucket=1536 confirmed as a real, measured ceiling) are all done, 153/153
regression, committed `41f7ed27ff`, not pushed. **Next, not started, not decided**:
whether the confirmed bucket=1536 ceiling is ever actually reached given the bounty
issue specifies no expected streaming duration (see that section above -- folded in with
the existing RTF/TTFP progress-note gap, one combined ask for the maintainers); HiFT's
`cache_source`/Hamming-crossfade streaming design (next round, read planned before code,
same discipline as the encoder/CFM rounds); and wiring any of this into a real streaming
call path (`flow.py`'s `inference`, plus the LLM's incremental `generate()`, still
untouched).
